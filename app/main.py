...
# ------------------------------------------------------------------
# 1. JSON MASTER REPORT ENDPOINT (FIXED ROUTE_ID VALIDATION)
# ------------------------------------------------------------------
...
def admin_report_range(
    ...
    route_id: str | None = Query(None), # Changed from int to str to handle empty strings "" safely
    db: Session = Depends(get_db),
):
    _require_admin(request, None)
    ...
    # Convert empty strings "" or invalid route strings from the frontend safely to None or int
    rid: int | None = None
    if route_id and route_id.strip() != "":
        try:
            rid = int(route_id)
        except ValueError:
            raise HTTPException(status_code=400, detail="Invalid route_id format. Must be an integer.")

    return _admin_report_range_payload(db, fd, td, rid)


# ------------------------------------------------------------------
# 2. PDF MASTER REPORT ENDPOINT (FIXED ROUTE_ID AND REMOVED DUPLICATE OUTPUT)
# ------------------------------------------------------------------
...
def admin_report_range_pdf(
    ...
    # Convert empty strings "" safely to None or int
    rid: int | None = None
    if route_id and route_id.strip() != "":
        try:
            rid = int(route_id)
    ...
    payload = _admin_report_range_payload(db, fd, td, rid)
    summary = payload["summary"]
    rows = payload["stores"]
    ...
    # Cleaned output stream (Removed the old crash-prone duplicate)
    raw = pdf.output()
...
# ------------------------------------------------------------------
# 3. CSV MASTER REPORT ENDPOINT (FIXED ROUTE_ID VALIDATION)
# ------------------------------------------------------------------
...
def admin_report_range_csv(
    ...
    for r in rows:
        w.writerow([
            ...
            r.get("returned") or 0,
            r.get("collected") or 0.0,
            r.get("balance") or 0.0,
        ])
    w.writerow([])
    w.writerow(["TOTAL", "", "", summary["total_delivered"], summary["total_returns"], summary["total_collected"], summary["total_balance"]])
